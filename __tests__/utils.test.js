...
describe("commentsFormatter", () => {
  ...
  it("returned object correctly changes belongs_to to article_id", () => {
    const comments = [
      {
        body:
  ...
  it("adds an author key to the comments object", () => {
    const comments = [
      {
        body:
          "Perferendis quia et nihil. Quasi ut voluptates sapiente et accusantium vel qui reprehenderit ratione. Autem alias voluptatem accusamus nesciunt beatae vero. Itaque repellat omnis et velit cum corporis aut id voluptas. Nostrum unde fuga ea. Perferendis quas maiores.",
    ...
    expect(
      commentsFormatter(comments, insertedArticles)[0].hasOwnProperty("author")
    ).toBe(true);
    expect(
      commentsFormatter(comments, insertedArticles)[0].hasOwnProperty(
        "created_by"
    ...
    expect(commentsFormatter(comments, insertedArticles)[0].author).toBe(
      "jessjelly"
    );
  });
});
